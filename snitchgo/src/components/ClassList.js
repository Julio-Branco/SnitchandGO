import { useEffect, useState } from "react";
import data from "../Data/Data.json";
import "../Styles/Class.css";

export default function ClassList() {
  const [selectedPerson, setSelectedPerson] = useState(null);
  const ClassListArray = data.personnes;

  const ShowUser = () => {
    return (
      <div className="UserStyle">
        {ClassListArray.map((personne, index) => (
          <div key={index} className="UserList">
            <img src={personne.pp} className="ProfilePic" alt="Profile" />
            <h3>{personne.nom}</h3>
            <h3>{personne.prenom}</h3>
            <button 
              className="btn btn-danger reportButton" 
              data-bs-toggle="modal"
              data-bs-target="#ModalReport"
              onClick={() => setSelectedPerson(personne)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                className="bi bi-megaphone"
                viewBox="0 0 16 16"
              >
                <path d="M13 2.5a1.5 1.5 0 0 1 3 0v11a1.5 1.5 0 0 1-3 0v-.214c-2.162-1.241-4.49-1.843-6.912-2.083l.405 2.712A1 1 0 0 1 5.51 15.1h-.548a1 1 0 0 1-.916-.599l-1.85-3.49-.202-.003A2.014 2.014 0 0 1 0 9V7a2.02 2.02 0 0 1 1.992-2.013 75 75 0 0 0 2.483-.075c3.043-.154 6.148-.849 8.525-2.199zm1 0v11a.5.5 0 0 0 1 0v-11a.5.5 0 0 0-1 0m-1 1.35c-2.344 1.205-5.209 1.842-8 2.033v4.233q.27.015.537.036c2.568.189 5.093.744 7.463 1.993zm-9 6.215v-4.13a95 95 0 0 1-1.992.052A1.02 1.02 0 0 0 1 7v2c0 .55.448 1.002 1.006 1.009A61 61 0 0 1 4 10.065m-.657.975 1.609 3.037.01.024h.548l-.002-.014-.443-2.966a68 68 0 0 0-1.722-.082z" />
              </svg>
              dénoncer
            </button>
          </div>
        ))}
      </div>
    );
  };

  return (
    <div className="ClassContainer">
      <ShowUser />

      {}
      <div
        className="modal fade"
        id="ModalReport"
        tabIndex="-1"
        aria-labelledby="profileModalLabel"
        aria-hidden="true"
      >
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="profileModalLabel">
                Balance ton collègue
              </h5>
              <button
                type="button"
                className="btn-close"
                data-bs-dismiss="modal"
                aria-label="Close"
              ></button>
            </div>
            <div className="modal-body">
              {selectedPerson && ( 
                <div>
                  <h6>Tu es en train de dénoncer : {selectedPerson.prenom} {selectedPerson.nom}</h6>
                </div>
              )}
              <input className="form-control" id="exampleFormControlInput1" placeholder="Matière"></input>
              <h5 style={{ color: "gray" }}>Raison :</h5>
              <div className="radioButtonReport">
                <div className="form-check form-check-inline">
                  <input className="form-check-input" type="radio" name="radioDenonce" id="radioRetard" value="R"></input>
                  <label className="form-check-label" htmlFor="radioRetard">
                    Retard
                  </label>
                </div>
                <div className="form-check form-check-inline">
                  <input className="form-check-input" type="radio" name="radioDenonce" id="radioAbsent" value="A"></input>
                  <label className="form-check-label" htmlFor="radioAbsent">
                    Absence
                  </label>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button
                type="button"
                className="btn btn-secondary"
                data-bs-dismiss="modal"
                style={{ backgroundColor: "green" }}
              >
                Valider
              </button>
              <button
                type="button"
                className="btn btn-secondary"
                data-bs-dismiss="modal"
              >
                Fermer
              </button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
